...
function callOnlyIfAclConfiguratorAvailable(func,thisObject){
    return function(){
        if(aclConfiguratorAvailable){
            func.apply(thisObject,arguments)
        }
        else{
            arguments[arguments.length-1](new Error("Acl configurator is not available"));
        }
    }
}
...
container.declareDependency("aclConfigurator", ["redisPersistence","redisClient"], function (outOfService, redisPersistence,redisClient) {
    ...
    return{
        'addRule':callOnlyIfAclConfiguratorAvailable(addRule),
        'removeRule' : callOnlyIfAclConfiguratorAvailable(removeRule),
        'getRules' : callOnlyIfAclConfiguratorAvailable(getRules),
        'addZoneParent' : callOnlyIfAclConfiguratorAvailable(addZoneParent),
        'delZoneParent': callOnlyIfAclConfiguratorAvailable(delZoneParent),
        'getRuleById':callOnlyIfAclConfiguratorAvailable(getRuleById)
    }
